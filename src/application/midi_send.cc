...
{
	  uart_send(0xb0 | sys_para[midi_c]);
	  uart_send(0x0);
	  uart_send(0x0);
	  uart_send(0x20);
	  uart_send(0x0);
...
TMidiSendTask::TMidiSendTask () :TTask()
{
}
...
//---------------------------------------------Midi event----------------------------------------
		  ...
		  {
			  trans_po++;
			  if(trans_po1 < trans_po)trans_po1 = trans_po;

			  tmp = midi_in_buf[trans_midi_po++];
			  if(tmp & 0x80)
			  {
				  data[0] = tmp ;
				  if(tmp != (sys_para[midi_c] | 0xc0))uart_send(tmp);
				  else pc_in_fl = 1;
				  ...
				  if(data[0] != (sys_para[midi_c] | 0xc0))uart_send(tmp);
				  if(contr_hader)
				  {
					  if(!contr_cont)
					  {
						...
						if(((data[0] & 0xf) == sys_para[midi_c]) && ((data[0] & 0xf0) == 0xb0))
						{
						    midi_b[1] = data[1];
						    midi_b[2] = tmp;
						    mid_fl = 1;
...
//-------------------------------------------Int event------------------------------------------
		  if(trans_contr_po != int_contr_po)
		  {
			  if(!contr_cont)
			  {
				  ...
				  if((data[0] & 0xf0) != 0xb0)uart_send(data[0]);
			  }
		  }
	  }
//-------------------------------------------Prog ch midi send------------------------------------
	  if(program_change_midi)
	  {
		  if(!contr_cont)
		  {
			  if(pc_in_fl)pc_in_fl = 0;
			  else {
				  send_bank();
			  }
			  uart_send(0xc0 | sys_para[midi_c]);
			  switch(control[510]){
			  case 0:if(midi_f1)uart_send(pc_in);else uart_send(prog);break;
			  case 1:uart_send(sys_para[prog + 128]);break;
			  case 2:uart_send(control[511] & 0x7f);break;
			  }
			  if((data[0] & 0xf0) != 0xc0)uart_send(data[0]);
...
//--------------------------------------------Foot11----------------------------------------------
	  if(key_midi1[0])
	  {
		  if(!contr_cont)
		  {
			  ...
			  if(key_midi1[0] == 2)uart_send(127);
			  else uart_send(0);
			  if((data[0] & 0xf0) != 0xb0)uart_send(data[0]);
			  key_midi1[0] = 0;
		  }
...
//--------------------------------------------Foot21----------------------------------------------
	  if(key_midi1[1])
	  {
	  	  if(!contr_cont)
	  	  {
	  		  ...
	  		  if(key_midi1[1] == 2)uart_send(127);
	  		  else uart_send(0);
	  		  if((data[0] & 0xf0) != 0xb0)uart_send(data[0]);
	  		  key_midi1[1] = 0;
	  	  }
...
//--------------------------------------------Foot31----------------------------------------------
	  if(key_midi1[2])
	  {
	  	  if(!contr_cont)
	  	  {
	  		  ...
	  		  if(key_midi1[2] == 2)uart_send(127);
	  		  else uart_send(0);
	  		  if((data[0] & 0xf0) != 0xb0)uart_send(data[0]);
	  		  key_midi1[2] = 0;
	  	  }
...
void adc_proc(void)
{
	...
	{
	  if(pc_mute_fl)ext_send(adc_bu2);
	}
	else {
	  ext_data = adc_bu2;
	  ext_sourc = 1;
...
extern "C" void ADC_IRQHandler()
{
  ...
  if(adc_bu_cont > 127)adc_bu_cont = 127;
  if(adc_inv_fl)adc_bu_cont = 127 - adc_bu_cont;
  ...
  if(adc_bu2 != adc_bu1[0])
  {
	 ...
	 if((sys_para[3] & 1))ADC_RegularChannelConfig(ADC1, 10, 1, ADC_SampleTime_480Cycles);
	 else ADC_RegularChannelConfig(ADC1, 11, 1, ADC_SampleTime_480Cycles);
