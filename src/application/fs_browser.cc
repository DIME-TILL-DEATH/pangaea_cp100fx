...
const char* fs_object_type_strings[] = { "File", "Dir", "Unknown" }  ;
...
{
  if ( curr_fs_object != fs_object_list.begin() ) curr_fs_object-- ;
...
{
  if ( curr_fs_object != fs_object_list.end()-1 ) curr_fs_object++ ;
...
//-----------------------------------------------------------------------------------------------
        ...
        if(cab_type != 2)
        {
        	if(file_size > 24576)file_size = 24576;
        }
        else {
        	if(file_size > 12288)file_size = 12288;
        }
        ...
        while(1)
        {
        	if(file_size > 512)
        	{
        		...
        		for(uint16_t i = 0 ; i < 512 ; i++)*buff++ = sd_buf[i];
        		file_size -= 512;
        	}
        	else {
        		f_read (&f, (void*)sd_buf, file_size , &br ) ;
        		for(uint16_t i = 0 ; i < file_size ; i++)*buff++ = sd_buf[i];
        		break;
        	}
        }
        // закрытие файла
...
void TFsBrowser::Load_cab(fs_object_t& object)
{
    ...
    if(!cab_num)gui_send(16,1);
    else gui_send(17,1);
    //send_codec(0xa301);
    while(object.name[name_point] && (name_point < 62))name_buf_temp[name_point + 1] = object.name[name_point++];
...
void TFsBrowser::Browse(const browse_command_t browse_command, fs_object_t& object, fs_object_list_t& object_list)
{
  ...
  switch ( browse_command )
  {
    case bcBack:
      ...
      {
    	  while ( curr_fs_object->name != ".." ) PrevObject( object );
