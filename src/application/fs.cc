          ...
          while(1)
            {
              ...
              if ( browse_command == TFsBrowser::bcFsMount || browse_command == TFsBrowser::bcFsUmount ||
            		  file_fl) continue ;
              if(browse_command != TFsBrowser::bcStartup)
              {
            	  Delay(10);
                  DisplayTask->Clear();
                  DisplayTask->StringOut(4,0,TDisplayTask::fntSystem,TDisplayTask::fnsBlack,(uint8_t*)FSTask->Object().dir.c_str());
                  DisplayTask->StringOut(4,1,TDisplayTask::fntSystem,TDisplayTask::fnsBlack,(uint8_t*)FSTask->Object().name.c_str());
              }
            }
	}
//------------------------------------------------------------------------------
