...
{
	default_param_handler(&presetData[fl], rl, args, count);
...
{
	default_param_handler(&presetData[FLANGER_MIX], rl, args, count);
...
{
	default_param_handler(&presetData[FLANGER_LFO], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_LFO_POS, presetData[FLANGER_LFO]);
}

static void flanger_rate_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&presetData[FLANGER_RATE], rl, args, count);
...
{
	default_param_handler(&presetData[FLANGER_WIDTH], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_WIDTH_POS, presetData[FLANGER_WIDTH]);
}

static void flanger_delay_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&presetData[FLANGER_DELAY], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_DELAY_POS, presetData[FLANGER_DELAY]);
}

static void flanger_feedback_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&presetData[FLANGER_FEEDBACK], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_FEEDBACK_POS, presetData[FLANGER_FEEDBACK]);
}

static void flanger_hpf_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&presetData[FLANGER_HPF], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_HPF_POS, presetData[FLANGER_HPF]);
}

static void flanger_position_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&presetData[FLANGER_PREPOST], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_FLANGER, FLANGER_PREPOST_POS, presetData[FLANGER_PREPOST]);
}
