...
static void ir_on_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	...
	DSP_contr_set_parameter(DSP_ADDRESS_MODULES_ENABLE, ENABLE_CAB, prog_data[ENABLE_CAB]);
}

static void ir_volume1_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_VOLUME1], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_VOLUME1_POS, prog_data[IR_VOLUME1]);
}

static void ir_pan1_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_PAN1], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_PAN1_POS, prog_data[IR_PAN1]);
}

static void ir_delay1_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_DELAY1], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_DELAY1_POS, prog_data[IR_DELAY1]);
}

static void ir_volume2_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_VOLUME2], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_VOLUME2_POS, prog_data[IR_VOLUME2]);
}

static void ir_pan2_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_PAN2], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_PAN2_POS, prog_data[IR_PAN2]);
}

static void ir_delay2_command_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	default_param_handler(&prog_data[IR_DELAY2], rl, args, count);
	DSP_contr_set_parameter(DSP_ADDRESS_CAB, IR_DELAY2_POS, prog_data[IR_DELAY2]);
}
