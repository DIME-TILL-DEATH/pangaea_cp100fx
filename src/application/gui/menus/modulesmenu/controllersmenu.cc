	...
	for(uint8_t i = 0; i < 3; i++)
	{
		...
		switch(i)
		{
			...
			case ControllerMenuParams::Source:
				...
				{
					DisplayTask->StringOut(45, 1, TDisplayTask::fntSystem, 0, &strControllerExt[currentPreset.controller[0].src - 1][0]);
				}
			break;

			case ControllerMenuParams::Destination:
				DisplayTask->StringOut(45, 2, TDisplayTask::fntSystem, 0, &strMidiDstList[currentPreset.controller[0].dst][0]);
			break;
		}
	}
	...
	if(!m_encoderKnobSelected)
	{
		if(m_parNum < paramsCount - 1)
		{
			...
			if(m_parNum == ControllerMenuParams::MinValue)
			{
				...
				DisplayTask->StringOut(0, 1, TDisplayTask::fntSystem, 0, &strControllersMenu[m_parNum + 1][0]);
				DisplayTask->StringOut(0, 2, TDisplayTask::fntSystem, 0, &strControllersMenu[m_parNum + 2][0]);
				DisplayTask->StringOut(0, 3, TDisplayTask::fntSystem, 0, &strControllersMenu[m_parNum + 3][0]);
				...
				DisplayTask->StringOut(45, 2, TDisplayTask::fntSystem, 0, &strOutPcList[currentPreset.pcOut][0]);
		...
		switch(m_parNum)
		{
			case ControllerMenuParams::Controller:
				if(m_controllerNum < controllersCount-1)
				{
					...
					{
						DisplayTask->StringOut(45, 1, TDisplayTask::fntSystem, 0, &strControllerExt[currentPreset.controller[m_controllerNum].src - 1][0]);
					}

					DisplayTask->StringOut(45, 2, TDisplayTask::fntSystem, 0, &strMidiDstList[currentPreset.controller[m_controllerNum].dst][0]);
	...
	if(!m_encoderKnobSelected)
	{
		if(m_parNum > 0)
		{
			...
			if(m_parNum == ControllerMenuParams::Destination)
			{
				DisplayTask->Clear();
				DisplayTask->Strel(58, 3, 0);
				DisplayTask->StringOut(0, 0, TDisplayTask::fntSystem, 0, &strControllersMenu[0][0]);
				DisplayTask->StringOut(0, 1, TDisplayTask::fntSystem, 0, &strControllersMenu[1][0]);
				for(uint8_t i = 0; i < 3; i++)
				{
					DisplayTask->StringOut(0, i, TDisplayTask::fntSystem, 0, &strControllersMenu[i][0]);
					switch(i)
