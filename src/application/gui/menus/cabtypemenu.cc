...
void CabTypeMenu::encoderClockwise()
{
	if(!m_paramNum)
	{
		DisplayTask->StringOut(6,par_num,TDisplayTask::fntSystem, 0, (uint8_t*)&cab_list_menu[m_paramNum++]);
		DisplayTask->StringOut(6,par_num,TDisplayTask::fntSystem, 2, (uint8_t*)&cab_list_menu[m_paramNum]);
	}
	tim5_start(0);
	clean_flag();
}
