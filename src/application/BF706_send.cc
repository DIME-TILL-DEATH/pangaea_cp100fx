...
void send_cab_data(uint8_t val,uint8_t num,uint8_t menu_fl)
{
	...
	for(uint32_t i = 0 ; i < 512 ; i++)
	{
	    ...
	    while(!SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE));
	    if(!menu_fl)SPI_I2S_SendData(SPI2 , prog_data[i]);
	    else SPI_I2S_SendData(SPI2 , preset_temp[i + 30]);
	}
	while(!SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE));
...
void gui_send(uint8_t num , uint16_t val)
{
	...
	case 6:Early_par(val);break;
	case 7:cab_volume(val);break;
	case 8:flanger_par(val);break;
	case 9:chor_par(val);break;
	case 10:tr_param(val);break;
	case 11:amp_volume(val);break;
	...
	case 22:eq_band_par(val);break;
	case 23:amp_imp_load();break;
	case 24:dsp_run(val);break;
	case 25:eq_mas_par(val);break;
	case 26:spdi_t(val);break;
	case 27:pr_par(val);break;
	case 28:cab_r_by(val);break;
	case 29:gate_param(val);break;
	case 30:comp_par(val);break;
	case 31:moog_par(val);break;
	case 32:dsp_mute(val);break;
	case 33:global_temp(val);break;
	}
	gui_fl = 0;
}
