...
{
	if(num==5)
		dsp_send(DSP_ADDRESS_CHORUS, num|(prog_data[hpf_ch]<<8));
	else
		dsp_send(DSP_ADDRESS_CHORUS, num|(prog_data[chor_volum+num]<<8));
}

void Phaser_par(uint8_t num)
{
	if(num==6)
		dsp_send(DSP_ADDRESS_PHASER, num|(prog_data[hpf_ph]<<8));
	else
	{
		if(num==7)
			dsp_send(DSP_ADDRESS_PHASER, num|(prog_data[phas_pos]<<8));
		else
			dsp_send(DSP_ADDRESS_PHASER, num|(prog_data[phaser_vol+num]<<8));
	}
}
...
void DSP_reverb_param(uint8_t num)
{
	...
	{
		case 0:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[r_vol]<<8));
		break;
		case 1:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[rev_t]<<8));
		break;
		case 8:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[rev_di]<<8));
		break;
		case 9:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[r_pre]<<8));
		break;
		case 10:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[r_tail]<<8));
		break;
		default:
			dsp_send(DSP_ADDRESS_REVERB, num|(prog_data[r_vol+num-1]<<8));
		break;
	}
}
...
void del_par(uint8_t num)
{
	...
	{
		case 0:
			dsp_send(DSP_ADDRESS_DELAY, 0|(prog_data[d_vol]<<8));
		break;
		case 1:
		...
		break;
		case 13:
			dsp_send(DSP_ADDRESS_DELAY, 13|(prog_data[d_tail]<<8));
		break;
		default:
			num--;
			dsp_send(DSP_ADDRESS_DELAY, num|(prog_data[d_vol+num]<<8));
		break;
	}
}
...
{
	if(num!=12)
		dsp_send(DSP_ADDRESS_EQ, num|(prog_data[eq1+num]<<8));
	else
		dsp_send(DSP_ADDRESS_EQ, num|(prog_data[eq_pr_po]<<8));
}
...
void tr_param(uint8_t num)
{
	if(num<5)
	{
		switch(num)
		{
			case 0:
			case 1:
				dsp_send(DSP_ADDRESS_TREMOLO, num|(prog_data[tr_vol+num]<<8));
			break;
			case 2:
				dsp_send(DSP_ADDRESS_TREMOLO, 6|(prog_data[tr_lfo_t]<<8));
			break;
			default:
				num--;
				dsp_send(DSP_ADDRESS_TREMOLO, num|(prog_data[tr_vol+num]<<8));
...
{
	if(num==6)
		dsp_send(DSP_ADDRESS_FLANGER, num|(prog_data[hpf_fl]<<8));
	else
	{
		if(num==7)
			dsp_send(DSP_ADDRESS_FLANGER, num|(prog_data[flan_pos]<<8));
		else
			dsp_send(DSP_ADDRESS_FLANGER, num|(prog_data[fl_v+num]<<8));
	}
}
...
{
	if(val<11)
		dsp_send(DSP_ADDRESS_RESONANCE_FILTER, val|(prog_data[mog_mix+val]<<8));
	else
	{
		if(val==11)
			dsp_send(DSP_ADDRESS_RESONANCE_FILTER, 12|(prog_data[mog_gen_t]<<8));
		else
		{
			if(val==12)
				dsp_send(DSP_ADDRESS_RESONANCE_FILTER, 11|(prog_data[mog_ext]<<8));
			else
			{
...
void send_cab_data(uint8_t val, uint8_t num, uint8_t menu_fl)
{
	...
	for(uint32_t i = 0; i<512; i++)
	{
		...
		while(!SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE));
		if(!menu_fl)
			SPI_I2S_SendData(SPI2, prog_data[i]);
		else
