...
static uint16_t CDC_Send (uint8_t* Buf, uint32_t Len);
	   uint16_t CDC_Recv (uint8_t* Buf, uint32_t Len);  // user defined function
...
/*
static uint16_t CDC_Recv (uint8_t* Buf, uint32_t Len)
{
	CDC_Send (Buf, Len) ;
  return USBD_OK;
}
